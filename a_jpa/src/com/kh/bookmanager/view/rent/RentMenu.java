	...
	public void rentMenu() {
		...
		do {
			...
			case 4: //대출건을 조회할 사용자의 아이디를 입력받아
				...
				//반환 받은 rentList를 출력
					System.out.println("대출건을 조회할 사용자의 아이디를 입력하세요 : ");
					userId = sc.next();
					...
					System.out.println("상세조회할 대출건이 있습니까?(y/n) : ");
					if(sc.next().equalsIgnoreCase("y")) {
						//사용자가 대출건 상세 조회를 하겠다고 하면
					    //상세 조회할 대출건 번호를 입력받고
					    //해당 대출건의 대출도서 목록을 출력
					    //출력 내용 : rmIdx, rbIdx, bIdx, 도서명, 반납일자, 반납여부
						System.out.println("상세조회 할 대출번호를 입력 : ");
						Long rmIdx = sc.nextLong();
						rents.stream()
							.filter(e->e.getRmIdx().equals(rmIdx)).findFirst().get()
