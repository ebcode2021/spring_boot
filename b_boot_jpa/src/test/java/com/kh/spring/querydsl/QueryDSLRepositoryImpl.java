...
public class QueryDSLRepositoryImpl implements QueryDSLRepositoryCustom{
	...
	public List<Rent> whereAnd(String title, String userId){
		//where메서드에 여러개의 Predicate를 전달할 수 있다. 이는 and연산과 동일하다.
		List<Rent> rents = query.select(rent)
					.from(rent)
					.where(rent.title.startsWith(title),rent.member.userId.eq(userId))
					.fetch();	
		return rents;
	}
...
//	List<Rent> rents = query.select(rent)
//			.from(rent)
//			.where(rent.title.startsWith("디디").and(rent.member.userId.eq("test")))
//			.fetch();	
	...
	@Override
	public List<Rent> innerJoinProjections(String userId) {
		//대출자 아이디가 test인 모든 대출건의 제목과 대출번호를 조회
	...
	public List<RentBook> thetaJoin() {
		...
		List<RentBook> theta = query.select(rentBook)
							.distinct()
							.from(rentBook)
	...
	public BooleanExpression rentCntLoe(int rentCnt) {
		return rentCnt == 0 ? null //where절의 파라미터에 null이 들어오면 무시하고 다음 조건절부터 쿼리를 작성
